...
def solve(path):
    ...
    def longest_distance(locations, source_location, prev_distance):
        ...
        return max(longest_distance(new_locations, location, prev_distance + distances[source_location, location]) for location in new_locations)

    print(longest_distance(locations, None, 0))


def main():
    solve('input-test.txt')  # 982
    solve('input.txt')  # 895
