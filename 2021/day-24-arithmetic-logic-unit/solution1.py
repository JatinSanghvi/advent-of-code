...
def solve(path):
    ...
    for line in lines:
        ...
        # Will retain largest inputs only, by virtue of sorting.
        values_dict = {values: input for values, input in sorted(values_tuples)}

    print(max(input for values, input in values_dict.items() if values.z == 0))
